...
read -s vault_pwd
echo $vault_pwd >"$VAULT_FOLDER/vault_pass_file"
while read name desc <&3; do
  echo "insert secret $name $desc"
  read -s value
  ansible-vault encrypt_string "$value" --name "$name" --vault-password-file "$VAULT_FOLDER/vault_pass_file" >>"$VAULT_FOLDER/$VAULT_FILE"
